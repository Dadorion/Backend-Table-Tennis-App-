...
class CompetitionService {
   ...
   //    const newCompetition = await pool.query('INSERT INTO competitions (fp_user_id, sp_user_id, date, tournament_id, location_id, winner) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [fp_user_id, sp_user_id, date, tournament_id, location_id, winner])
   ...
   async getOne(id) {
      ...
      const answer = await pool.query('SELECT * FROM competitions WHERE id = $1', [id])
      return answer.rows[0]
   }
   async update(competition) {
      const { fp_user_id, sp_user_id, date, tournament_id, location_id, winner, id } = competition

      const updetedCompetition = await pool.query('UPDATE competitions SET fp_user_id = $1, sp_user_id = $2, date = $3, tournament_id = $4, location_id = $5, winner = $6 WHERE id = $7 RETURNING *', [fp_user_id, sp_user_id, date, tournament_id, location_id, winner, id])
   ...
   async delete(id) {
      ...
      const deletedCompetition = await pool.query('DELETE FROM competitions WHERE id = $1 RETURNING *', [id])
