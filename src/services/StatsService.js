...
class StatsService {
  calculateStats(matches, matchesIds) {
    ...
    const winsPercent =
      total > 0 ? ((winsCount / total) * 100).toFixed(2) : "пока нет побед";
  ...
  async getOne(id) {
    const matchesIds = await pool.query(
      "SELECT DISTINCT matches.id FROM matches JOIN players_matches ON players_matches.match_id = matches.id JOIN players ON players.id = players_matches.player_id WHERE players.id = $1",
      [id]
    );
