...
import playerMatchesRespBuilder from '../responceBuilders/playerMatch_respBuilder .js'
...
class MatchService {
   async create(match) {
      ...
      const qMatch = Query.insert('matches', ['is_important', 'date', 'location_id', 'player_id'])
   ...
   async playerMatches(id) {
      if (!id) {
         throw new Error('Получение матчей игрока не удалось. Не указан ID игрока.')
      }
      const q = `
      SELECT
         m.id AS m_id,
      ...
      // Здесь обработать ответ от базы и упаковать в удобочитаемый формат.
